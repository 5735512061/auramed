...
if( !function_exists('sirpi_archive_blog_post_defaults') ) {
    function sirpi_archive_blog_post_defaults() {
        $defaults = array(
...
if( !function_exists('sirpi_archive_blog_post_misc_defaults') ) {
    function sirpi_archive_blog_post_misc_defaults() {
        $defaults = array(
...
if( !function_exists('sirpi_archive_blog_post_params_default') ) {
    function sirpi_archive_blog_post_params_default() {
        $params = array(
            'enable_post_format'   	 => 0,
            'enable_video_audio' 	 => 0,
            'enable_gallery_slider'  => 1,
            'archive_post_elements'  => array( 'feature_image', 'date', 'meta_group', 'title', 'content', 'read_more' ),
            'archive_meta_elements'  => array( 'comment' ),
            'archive_readmore_text'  => esc_html__('Read More', 'sirpi'),
...
if( !function_exists('sirpi_archive_blog_post_defaults_filter') ) {
    function sirpi_archive_blog_post_defaults_filter() {
        $defaults = sirpi_archive_blog_post_defaults();
        return apply_filters( 'sirpi_archive_post_cmb_class', $defaults );
    }
}

if( !function_exists('sirpi_archive_blog_post_misc_defaults_filter') ) {
    function sirpi_archive_blog_post_misc_defaults_filter() {
        $defaults = sirpi_archive_blog_post_misc_defaults();
        return apply_filters( 'sirpi_archive_post_hld_class', $defaults );
    }
}

if( !function_exists('sirpi_archive_blog_post_params') ) {
    function sirpi_archive_blog_post_params() {
        $params = sirpi_archive_blog_post_params_default();
        return apply_filters( 'sirpi_archive_blog_post_params', $params );
    }
}

if( !function_exists('sirpi_get_archive_post_combine_class') ) {
	function sirpi_get_archive_post_combine_class() {

        $blog_defaults = sirpi_archive_blog_post_defaults_filter();
        ...
        return apply_filters( 'sirpi_get_archive_post_combine_class', implode( ' ', $combine_class ) );
	}
}

if( !function_exists('sirpi_get_archive_post_holder_class') ) {
	function sirpi_get_archive_post_holder_class() {

        $blog_defaults = sirpi_archive_blog_post_defaults_filter();
        $blog_misc_defaults = sirpi_archive_blog_post_misc_defaults_filter();
        ...
        return apply_filters( 'sirpi_get_archive_post_holder_class', implode( ' ', $holder_class ) );
	}
}

if( !function_exists('sirpi_get_archive_post_style') ) {
	function sirpi_get_archive_post_style() {
		...
		return apply_filters( 'sirpi_get_archive_post_style', $post_style );
	}
}

if( !function_exists('sirpi_get_archive_post_column') ) {
	function sirpi_get_archive_post_column() {
		...
		return apply_filters( 'sirpi_get_archive_post_column', $post_columns );
	}
}

add_filter('post_class', 'sirpi_archive_set_post_class', 10, 3);
if( !function_exists('sirpi_archive_set_post_class') ) {
    function sirpi_archive_set_post_class( $classes, $class, $post_id ) {
        ...
        if( is_post_type_archive('post') || is_search() || is_category() || is_tag() || is_home() || is_author() || is_year() || is_month() || is_day() || is_time() || is_tax('post_format') || ( defined('DOING_AJAX') && DOING_AJAX ) ) {
            ...
            $blog_params = sirpi_archive_blog_post_params();
...
add_action( 'sirpi_after_main_css', 'sirpi_blog_enqueue_css', 10 );
if( !function_exists( 'sirpi_blog_enqueue_css' ) ) {
	function sirpi_blog_enqueue_css() {
		wp_enqueue_style( 'wdt-blog', get_theme_file_uri('/modules/blog/assets/css/blog.css'), false, SIRPI_THEME_VERSION, 'all');
		...
		wp_enqueue_style( 'jquery-bxslider', get_theme_file_uri('/modules/blog/assets/css/jquery.bxslider.css'), false, SIRPI_THEME_VERSION, 'all' );
	}
}

add_action( 'sirpi_before_enqueue_js', 'sirpi_blog_enqueue_js' );
if( !function_exists( 'sirpi_blog_enqueue_js' ) ) {
	function sirpi_blog_enqueue_js() {
...
if( !function_exists( 'after_blog_post_content_pagination' ) ) {
    function after_blog_post_content_pagination() {
    	...
    	$pagination_template = sirpi_archive_blog_post_params();
    	$pagination_template = $pagination_template['archive_blog_pagination'];

        echo apply_filters( 'sirpi_blog_archive_pagination', sirpi_get_template_part( 'pagination', 'templates/'.$pagination_template ) );
    }
    add_action( 'sirpi_after_blog_post_content_wrap', 'after_blog_post_content_pagination' );
}

if( !function_exists( 'sirpi_excerpt' ) ) {
	function sirpi_excerpt( $limit = NULL ) {
