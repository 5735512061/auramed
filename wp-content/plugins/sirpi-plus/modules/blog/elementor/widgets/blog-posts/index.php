...
if( !class_exists( 'SirpiPlusBlogPostsWidget' ) ) {
    class SirpiPlusBlogPostsWidget {
        ...
        function register_widgets( $widgets_manager ) {
            require SIRPI_PLUS_DIR_PATH. 'modules/blog/elementor/widgets/blog-posts/class-widget-blog-posts.php';
        ...
        public function register_call_blog_elementor_sc_filters( $settings ) {
            ...
            add_filter( 'sirpi_blog_archive_elem_order_params', array( $this, 'register_blog_archive_elem_order_params' ), 20, 1 );
        }
        ...
        public function blog_elementor_sc_load_more_post() {
            ...
            $blogpostloadmore_nonce = sirpi_sanitization($_POST['blogpostloadmore_nonce']);
            if( isset( $blogpostloadmore_nonce ) && wp_verify_nonce( $blogpostloadmore_nonce, 'blogpostloadmore_nonce' ) ) {

                $parse_str = ( isset( $_REQUEST['settings'] ) ) ? parse_str( sirpi_sanitization($_REQUEST['settings']), $settings ) : '';
                $this->blog_settings = $settings;

                $count = ( isset( $_REQUEST['count'] ) ) ? sirpi_sanitization($_REQUEST['count']) : get_option( 'posts_per_page' );
                $page  = ( isset( $_REQUEST['pageNumber'] ) ) ? sirpi_sanitization($_REQUEST['pageNumber']) : 2;
                $cats  = ( isset( $_REQUEST['cats'] ) ) ? explode( ',', sirpi_sanitization($_REQUEST['cats']) ) : '';
                    ...
                    while( $the_query->have_posts() ) :
                        ...
                        <div class="<?php echo esc_attr($combine_class);?>">
                            <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>><?php
                                ...
                                sirpi_template_part( 'blog', 'templates/'.esc_attr($post_style).'/post', '', $template_args ); ?>
                            </article>
                        </div><?php
                    endwhile;
                    wp_reset_postdata();
        ...
        function register_widget_styles() {
            wp_register_style( 'swiper',
                SIRPI_PLUS_DIR_URL . 'modules/blog/elementor/widgets/assets/css/swiper.min.css', array(), SIRPI_PLUS_VERSION );
            wp_register_style( 'wdt-blogcarousel',
                SIRPI_PLUS_DIR_URL . 'modules/blog/elementor/widgets/assets/css/blogcarousel.css', array(), SIRPI_PLUS_VERSION );
        }

        function register_widget_scripts() {
            wp_register_script( 'jquery-swiper',
                SIRPI_PLUS_DIR_URL . 'modules/blog/elementor/widgets/assets/js/swiper.min.js', array( 'jquery' ), SIRPI_PLUS_VERSION, true );
            wp_register_script( 'wdt-blogcarousel',
                SIRPI_PLUS_DIR_URL . 'modules/blog/elementor/widgets/assets/js/blogcarousel.js', array(), SIRPI_PLUS_VERSION, true );
        }
...
SirpiPlusBlogPostsWidget::instance();