...
class Sirpi_Shop_Widget_Menu_Icon extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Menu Icon', 'sirpi-pro' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'cart_icon_section', array(
			'label' => esc_html__( 'Cart Icon', 'sirpi-pro' ),
		) );

			$this->add_control( 'cart_action', array(
				'label'       => esc_html__( 'Cart Action', 'sirpi-pro' ),
				'type'        => Controls_Manager::SELECT,
				'description' => esc_html__( 'Choose how you want to display the cart content.', 'sirpi-pro'),
				'default'     => '',
				'options'     => array(
		...
		$output .= '<div class="wdt-shop-menu-icon '.$settings['class'].'">';
			...
			if($settings['cart_action'] == 'notification_widget') {
				...
				$output .= '<div class="wdt-shop-menu-cart-content-wrapper">';
					$output .= '<div class="wdt-shop-menu-cart-content">'.esc_html__('No products added!', 'sirpi-pro').'</div>';
				$output .= '</div>';
