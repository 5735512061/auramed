...
if( !class_exists( 'SirpiProBCTemplate' ) ) {
    class SirpiProBCTemplate {
        ...
        function fonts_list( $fonts ) {
            $title = sirpi_customizer_frontend_font( sirpi_customizer_settings('breadcrumb_title_typo'), array() );
            if( count( $title ) ) {
                array_push( $fonts, $title[0] );
            }

            $content = sirpi_customizer_frontend_font( sirpi_customizer_settings('breadcrumb_typo'), array() );
            if( count( $content ) ) {
        ...
        function base_style( $style ) {
            ...
            $bg                = sirpi_customizer_settings('breadcrumb_background');
            $enable_overlay    = sirpi_customizer_settings('breadcrumb_overlay_bg_color');
            $gradient_color    = sirpi_customizer_settings('breadcrumb_background_gradient_color');
            $title_typo        = sirpi_customizer_settings('breadcrumb_title_typo');
            $title_color       = sirpi_customizer_settings('breadcrumb_title_color');
            $content_typo      = sirpi_customizer_settings('breadcrumb_typo');
            $content_color     = sirpi_customizer_settings('breadcrumb_text_color');
            $content_a_color   = sirpi_customizer_settings('breadcrumb_link_color');
            $content_a_h_color = sirpi_customizer_settings('breadcrumb_link_hover_color');
            ...
            if( !empty( $title_css ) ) {
                $style .= sirpi_customizer_dynamic_style( '.dark-bg-breadcrumb .main-title-section h1,  .main-title-section h1', $title_css );
            }
            ...
            if( !empty( $content_css ) ) {
                $style .= sirpi_customizer_dynamic_style( '.dark-bg-breadcrumb .breadcrumb, .dark-bg-breadcrumb .breadcrumb span.current, .breadcrumb, .breadcrumb span.current', $content_css );
            }

            $content_a_css = sirpi_customizer_color_settings( $content_a_color );
            if( !empty( $content_a_css ) ) {
                $style .= sirpi_customizer_dynamic_style( '.dark-bg-breadcrumb .breadcrumb a, .breadcrumb a', $content_a_css );
            }

            $content_a_h_css = sirpi_customizer_color_settings( $content_a_h_color );
            if( !empty( $content_a_h_css ) ) {
                $style .= sirpi_customizer_dynamic_style( '.dark-bg-breadcrumb .breadcrumb a:hover, .breadcrumb a:hover', $content_a_h_css );
            }

            if(is_singular()) {
                global $post;
                $post_meta = get_post_meta( $post->ID, '_sirpi_breadcrumb_settings', true );
            ...
            if( !empty( $bg_css ) && empty( $enable_overlay ) ) {
                ...
                $style .= sirpi_customizer_dynamic_style( '.main-title-section-wrapper.overlay-wrapper.dark-bg-breadcrumb > .main-title-section-bg, .main-title-section-wrapper.overlay-wrapper > .main-title-section-bg, .main-title-section-wrapper.dark-bg-breadcrumb > .main-title-section-bg, .main-title-section-wrapper > .main-title-section-bg', $bg_css );
                ...
                if( !empty( $overlay_color ) || !empty( $overlay_gradient_color ) ) {
                    $bg_css = sirpi_customizer_bg_settings( array( 'background-color' => $overlay_color, 'gradient-background-color' => $overlay_gradient_color, 'breadcrumb_overlay_bg_color' => true ) );
        ...
        function tablet_landscape_style( $style ) {
            ...
            if( !empty( $title_typo_css) ) {
                $style .= sirpi_customizer_dynamic_style( '.dark-bg-breadcrumb .main-title-section h1,  .main-title-section h1', $title_typo_css );
            }

            $content_typo     = sirpi_customizer_settings('breadcrumb_typo');
            $content_typo_css = sirpi_customizer_responsive_typography_settings( $content_typo, 'tablet-ls' );
            if( !empty( $content_typo_css) ) {
                $style .= sirpi_customizer_dynamic_style( '.breadcrumb', $content_typo_css );
            }
