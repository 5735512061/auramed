...
class Elementor_Side_Nav_Navigation extends SirpiElementorWidgetBase {
    ...
    public function get_title() {
        return esc_html__('Side Nav - Navigation', 'sirpi-pro');
    }
        ...
        $output .= '<div class="wdt-sidenav-navigation-container '.esc_attr( $el_class ).'">';
            ...
            if($prev_page_id >= 0) {
                ...
                $output .= '<div class="wdt-sidenav-navigation-prev-wrapper">';
                    ...
                    $output .= '<div class="wdt-sidenav-navigation-title-wrapper">';
                        $output .= '<p><a href="'.get_permalink($prev_page_id).'" title="'.esc_attr(get_the_title($prev_page_id)).'"><i class="wdticon-angle-left"></i>'.esc_html__('Previous Service','sirpi-pro').'</a></p>';
                        ...
                        $output .= '<h3><a href="'.get_permalink($prev_page_id).'" title="'.esc_attr(get_the_title($prev_page_id)).'">';
                            if(get_the_title($prev_page_id)) {
                                ...
                                $output .= esc_html__('Previous Page', 'sirpi-pro');
                            }
                        $output .= '</a></h3>';
                    $output .= '</div>';
                $output .= '</div>';
                ...
                $output .= '<div class="wdt-sidenav-navigation-prev-wrapper no-post">';
                    $output .= '<a href="#" class="wdt-sidenav-navigation-prev-bgimg"><i class="wdticon-angle-left"></i></a>';
                    $output .= '<div class="wdt-sidenav-navigation-title-wrapper">';
                        $output .= '<span class="zmdi zmdi-long-arrow-left zmdi-hc-fw"></span>';
                        $output .= '<h3>'.esc_html__('No previous service to show!', 'sirpi-pro').'</h3>';
                    $output .= '</div>';
                $output .= '</div>';
            ...
            if($next_page_id >= 0) {
                ...
                $output .= '<div class="wdt-sidenav-navigation-next-wrapper">';
                    ...
                    $output .= '<div class="wdt-sidenav-navigation-title-wrapper">';
                        $output .= '<p><a href="'.get_permalink($next_page_id).'" title="'.esc_attr(get_the_title($next_page_id)).'">'.esc_html__('Next Service','sirpi-pro').'<i class="wdticon-angle-right"></i></a></p>';
                        $output .= '<span class="zmdi zmdi-long-arrow-left zmdi-hc-fw"></span>';
                        $output .= '<h3><a href="'.get_permalink($next_page_id).'" title="'.esc_attr(get_the_title($next_page_id)).'">';
                            if(get_the_title($next_page_id)) {
                                ...
                                $output .= esc_html__('Next Page', 'sirpi-pro');
                            }
                        $output .= '</a></h3>';
                    $output .= '</div>';
                $output .= '</div>';
                ...
                $output .= '<div class="wdt-sidenav-navigation-next-wrapper no-post">';
                    $output .= '<a href="#" class="wdt-sidenav-navigation-next-bgimg"><i class="wdticon-angle-right"></i></a>';
                    $output .= '<div class="wdt-sidenav-navigation-title-wrapper">';
                        $output .= '<span class="zmdi zmdi-long-arrow-left zmdi-hc-fw"></span>';
                        $output .= '<h3>'.esc_html__('No next service to show!', 'sirpi-pro').'</h3>';
                    $output .= '</div>';
                $output .= '</div>';
