<?php
class Sirpi_Widget_Recent_Posts extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array(
			'classname'   => 'widget_recent_posts',
			'description' => esc_html__('To list out posts', 'sirpi-pro')
		);

        $theme_name =  defined('SIRPI_THEME_NAME') ? SIRPI_THEME_NAME : 'Sirpi';
		parent::__construct( false, $theme_name . esc_html__(' Posts','sirpi-pro'), $widget_options );
	}
	...
	function form($instance) {
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('title')); ?>">
        		<?php esc_html_e('Title:','sirpi-pro');?>
	    ...
	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('_post_categories')); ?>">
	    		<?php esc_html_e('Choose the categories you want to display (multiple selection possible)','sirpi-pro');?>
	    	</label>
            <select class="widefat" id="<?php echo esc_attr($this->get_field_id('_post_categories').'[]');?>" name="<?php echo esc_attr($this->get_field_name('_post_categories').'[]');?>" multiple="multiple">
            	<option value=""><?php esc_html_e("Select",'sirpi-pro');?></option><?php
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('_excerpt')); ?>">
        		<?php esc_html_e('Display title only or title &amp; excerpt','sirpi-pro');?>
        	</label>
        ...
        <p>
        	...
        	<?php esc_html_e("Show Image",'sirpi-pro');?>
        </p>

	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('_post_count'));?>">
	    		<?php esc_html_e('No.of posts to show:','sirpi-pro');?>
	    	</label>
	...
	function widget($args, $instance) {
		...
		echo sirpi_pro_before_after_widget( $before_widget );

		if( !empty( $title ) ) {

			echo sirpi_pro_widget_title( $before_title . $title . $after_title );
		}
			 ...
			 while( $the_query->have_posts() ):
				...
				echo "<li>";
					if(1 == $_enabled_image):
						...
						$image = ( $image != false)? $image[0] : SIRPI_PRO_DIR_URL . 'modules/blog/assets/images/post-thumb.jpg';
						echo "<div class='entry-image'><a href='".get_permalink()."' class='thumb'>";
							echo "<img src='{$image}' alt='".the_title_attribute('echo=0')."'>";
						echo "</a></div>";
					endif;

					echo '<div class="widget-meta-group-content">';
						if($show_title):
							echo "<div class='entry-title'><h4><a href='".get_permalink()."'>{$title}</a></h4></div>";
						else:
							echo "<div class='entry-title'><h4><a href='".get_permalink()."'>{$title}</a></h4></div>";
							echo "<div class='entry-content'>".sirpi_excerpt(10)."</div>";
						endif;

						echo '<div class="entry-meta">';
							echo '<p> <span class="wdticon-clock-o"> </span> '.get_the_date ( get_option('date_format') ).'</p>';
							$commtext = "";
							if( ( wp_count_comments( $post->ID )->approved ) == 0 )	$commtext = '0';
							else $commtext = wp_count_comments( $post->ID )->approved;
							echo '<p> <a href="'.get_permalink().'#comments"><span class="wdticon-comment"> </span> '.$commtext.'</a></p>';
						echo '</div>';
					echo '</div>';
				echo "</li>";
			 endwhile;
			 else:
			 	echo "<li><h4>".esc_html__('No Posts found','sirpi-pro')."</h4></li>";
			 endif;
			 wp_reset_postdata();
	 	echo "</ul></div>";

		echo sirpi_pro_before_after_widget( $after_widget );
	}
}?>