...
class WeDesignTech_Widget_Base_Thumbs_Slider {
	...
	public function name() {
		return 'wdt-thumb-slider';
	}

	public function title() {
		return esc_html__( 'Thumb Slider', 'wdt-elementor-addon' );
	}
	...
	public function init_styles() {
		return array (
			'swiper-css' => WEDESIGNTECH_ELEMENTOR_ADDON_DIR_URL . 'inc/widgets/thumb-slider/assets/css/swiper.min.css',
            $this->name() =>  WEDESIGNTECH_ELEMENTOR_ADDON_DIR_URL.'inc/widgets/thumb-slider/assets/css/style.css'	
		);
	}
	...
	public function init_scripts() {
		return array (
			'swiper-js' => WEDESIGNTECH_ELEMENTOR_ADDON_DIR_URL . 'inc/widgets/thumb-slider/assets/js/swiper.min.js',
			$this->name() => WEDESIGNTECH_ELEMENTOR_ADDON_DIR_URL.'inc/widgets/thumb-slider/assets/js/script.js'
	...
	public function create_elementor_controls($elementor_object) {
		...
		$elementor_object->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .wdt-thumb-slider-container, {{WRAPPER}} .wdt-thumb-slider-container h5, {{WRAPPER}} .wdt-thumb-slider-container a, {{WRAPPER}} .wdt-thumb-slider-sub-title h6' => 'color: {{VALUE}}',
				),
			)
		);
		...
		$elementor_object->add_control(
			...
			array(
				...
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .wdt-thumb-slider-container:hover, {{WRAPPER}} .wdt-thumb-slider-container:hover h5, {{WRAPPER}} .wdt-thumb-slider-container:hover a, {{WRAPPER}} .wdt-thumb-slider-sub-title:hover h6' => 'color: {{VALUE}}',
				),
			)
		);
		...
		$elementor_object->add_group_control(
			...
			array(
				...
				'selector' => '{{WRAPPER}} .wdt-thumb-slider-title h5',
			)
		);
		...
		$elementor_object->add_responsive_control(
			...
			array (
				...
				'size_units' => array ( 'px', 'em', '%' ),
				'selectors' => array (
					'{{WRAPPER}} .wdt-thumb-slider-title h5' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		...
		$elementor_object->add_control(
			...
			array(
				...
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .wdt-thumb-slider-container h5' => 'color: {{VALUE}}',
				),
			)
		);
		...
		$elementor_object->add_control(
			...
			array(
				...
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .wdt-thumb-slider-container:hover h5' => 'color: {{VALUE}}',
				),
			)
		);
		...
		$elementor_object->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .wdt-thumb-slider-icon-container i' => 'font-size: {{SIZE}}{{UNIT}};',
				),
			)
		);
		...
		$elementor_object->add_control(
			...
			array(
				...
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .wdt-thumb-slider-button a' => 'color: {{VALUE}}',
				),
			)
		);
		...
		$elementor_object->add_control(
			...
			array(
				...
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .wdt-thumb-slider-button a:hover' => 'color: {{VALUE}}',
				),
			)
		);
	...
	public function render_html($widget_object, $settings) {
		...
		$settings['module_class'] = 'carousel-advanced-slider-thumb-';
	
		$settings_attr = $this->get_thumb_carousel_attributes($settings);

		$output .= '<div class="wdt-advanced-thumb-carousel-holder" data-settings="'.esc_attr($settings_attr).'">';
			$output .= '<div class="swiper swiper-container wdt-'.esc_attr($settings['module_class']).($settings['module_id']).'" data-id="'.($settings['module_id']).'" data-wrapper-class="wdt-'.esc_attr($settings['module_class']).($settings['module_id']).'">';
				...
				$output .= '<div class="swiper-wrapper">';
					...
					foreach ( $settings['features_content'] as $index => $item ) :
						...
						if( $item['content_type'] == 'template' ) {
							...
							$output .='<div class="wdt-thumb-slider-container">';
								...
								if(!empty($item['list_icon']['value'])) {
									...
									$output .= '</div>';
										...
										if($item['list_title'] || $item['list_sub_title'] || $item['list_content']  != '') {
											$output .= '<div class="wdt-thumb-slider-title"> <h5>' . $item['list_title'] . '</h5></div>';
					...
					if ( $settings['arrows'] == 'yes' ) :
						$output .= '<div class="wdt-thumbcarousel-arrow-pagination">';
							if(!empty( $settings['arrows_prev_icon']['value'])) {
								$output .= '<div class="wdt-arrow-advanced-thumb-pagination-prev wdt-arrow-advanced-thumb-pagination-prev-'.esc_attr($settings['module_id']).'">';
									$output .= ($settings['arrows_prev_icon']['library'] === 'svg') ? '<i>' : '';
										...
										$output .= ob_get_clean();
									$output .= ($settings['arrows_prev_icon']['library'] === 'svg') ? '</i>' : '';
									$output .= '<span>'.esc_html__('Previous', 'wdt-elementor-addon').'</span>';
								$output .= '</div>';
							}
							if(!empty($settings['arrows_next_icon']['value'])) {
								$output .= '<div class="wdt-arrow-advanced-thumb-pagination-next wdt-arrow-advanced-thumb-pagination-next-'.esc_attr($settings['module_id']).'">';
								$output .= '<span>'.esc_html__('Next', 'wdt-elementor-addon').'</span>';
			...
			$output .= '<div class="swiper swiper-container wdt-advanced-thumbnail-carousel wdt-thumbnail-carousel-'.($settings['module_id']).'" data-wrapper-advanced-thumb-class="wdt-thumbnail-carousel-'.($settings['module_id']).'">';
...
if( !function_exists( 'wedesigntech_widget_base_advanced_thumbs_slider' ) ) {
    function wedesigntech_widget_base_advanced_thumbs_slider() {
