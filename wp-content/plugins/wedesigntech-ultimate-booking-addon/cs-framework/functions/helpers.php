...
if ( ! function_exists( 'cs_add_element' ) ) {
  function cs_add_element( $field = array(), $value = '', $unique = '' ) {
    ...
    if( class_exists( $class ) ) {
      ...
      $output .= '<p>'. esc_html__( 'This field class is not available!', 'wedesigntech-ultimate-booking-addon' ) .'</p>';
    }
...
if ( ! function_exists( 'cs_get_var' ) ) {
  function cs_get_var( $var, $default = '' ) {
    ...
    if( isset( $_POST[$var] ) ) {
      return ultimate_booking_pro_sanitization($_POST[$var]);
    }

    if( isset( $_GET[$var] ) ) {
      return ultimate_booking_pro_sanitization($_GET[$var]);
    }
...
if ( ! function_exists( 'cs_get_vars' ) ) {
  function cs_get_vars( $var, $depth, $default = '' ) {
    ...
    if( isset( $_POST[$var][$depth] ) ) {
      return ultimate_booking_pro_sanitization($_POST[$var][$depth]);
    }

    if( isset( $_GET[$var][$depth] ) ) {
      return ultimate_booking_pro_sanitization($_GET[$var][$depth]);
    }
